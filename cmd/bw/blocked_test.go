...
func TestCmdBlockedBasic(t *testing.T) {
	...
	err := cmdBlocked(env.Store, []string{}, PlainWriter(&buf))
	if err != nil {
...
func TestCmdBlockedJSON(t *testing.T) {
	...
	var buf bytes.Buffer
	err := cmdBlocked(env.Store, []string{"--json"}, PlainWriter(&buf))
	if err != nil {
...
func TestCmdBlockedUnknownFlag(t *testing.T) {
	...
	var buf bytes.Buffer
	err := cmdBlocked(env.Store, []string{"--verbose"}, PlainWriter(&buf))
	if err == nil {

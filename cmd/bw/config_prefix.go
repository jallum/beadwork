...
func renamePrefix(store *issue.Store, r *repo.Repo, newPrefix string, w Writer) error {
	...
	collisionRepo := ""
	if oldPrefix != "" {
		collisionRepo = otherRepoClaiming(r, oldPrefix)
	}
	...
	// The host-local registry will pick up the new Prefix + Aliases on
	// the next touchRegistry (which fires at the end of main dispatch).
	// Other machines that interact with the repo learn the aliases
	// automatically since they're stored in .bwconfig on the beadwork
	// branch.
...
func otherRepoClaiming(r *repo.Repo, prefix string) string {
	selfPath, err := registry.CanonicalRepoPath(r.RepoDir())
	if err != nil {
		return ""
	}
	...
	for _, path := range reg.LookupPrefix(prefix) {
		if path != selfPath {
			return path
		}
	}

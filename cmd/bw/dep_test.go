...
func TestCmdDepAddBasic(t *testing.T) {
	...
	err := cmdDepAdd(env.Store, []string{a.ID, "blocks", b.ID}, PlainWriter(&buf))
	if err != nil {
...
func TestCmdDepAddNotFound(t *testing.T) {
	...
	var buf bytes.Buffer
	err := cmdDepAdd(env.Store, []string{"nonexistent", "blocks", "also-missing"}, PlainWriter(&buf))
	if err == nil {
...
func TestCmdDepRemoveBasic(t *testing.T) {
	...
	var buf bytes.Buffer
	err := cmdDepRemove(env.Store, []string{a.ID, "blocks", b.ID}, PlainWriter(&buf))
	if err != nil {
...
func TestCmdDepRemoveNotFound(t *testing.T) {
	...
	var buf bytes.Buffer
	err := cmdDepRemove(env.Store, []string{"nonexistent", "blocks", "also-missing"}, PlainWriter(&buf))
	if err == nil {
		t.Error("expected error for nonexistent issues")
	}
}
